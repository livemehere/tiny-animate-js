...
class Tween {
  constructor({ selector, startProps, endProps, duration, ease, delay }) {
    this.eases = { "power4.out": "cubic-bezier(0.08, 1, 0.46, 1)" };
    this.selector = selector;
    this.startProps = startProps ?? {};
    this.endProps = endProps ?? {};
    this.duration = duration ?? 1;
    this.ease = ease ?? this.eases["power4.out"];
    this.delay = delay ?? 0;
    this.animate = null;
  }
  initFrom(el, startProps) {
    ...
    el.style.transform = `translate(${this.setValueWithUnit(
      this.startProps.x,
    )}, ${this.setValueWithUnit(this.startProps.y)}) scale(${
      this.startProps.scale || 1
    }) rotate(${this.startProps.rotate || 0})`;
  }
  initTo(el, endProps) {
    const originStyle = getComputedStyle(el);
    ...
    el.style.transform = `translate(${this.setValueWithUnit(
      this.endProps.x,
    )}, ${this.setValueWithUnit(this.endProps.y)}) scale(${
      this.endProps.scale || 1
    }) rotate(${this.endProps.rotate || 0})`;
  }
  initFromTo(el, startProps) {
    el.style.transformOrigin = "center";
    el.style.transform = `translate(${this.setValueWithUnit(
      startProps.x,
    )}, ${this.setValueWithUnit(startProps.y)}) scale(${
      startProps.scale || 1
    }) rotate(${startProps.rotate || 0})`;
      ...
      [
        {
          ...
          transform: `translate(${this.setValueWithUnit(
            x,
          )}, ${this.setValueWithUnit(y)}) scale(${scale || 1}) rotate(${
            rotate || 0
          }deg)`,
          opacity: opacity ?? 1,
        },
      ...
      [
        { ...this.startProps },
        {
          ...
          transform: `translate(${this.setValueWithUnit(
            ...
            this.endProps.scale || 1
          }) rotate(${this.endProps.rotate || 0}deg)`,
          opacity: this.endProps.opacity ?? 1,
        },
      ],
      { ...this.getDefaultOption() },
...
class Timeline {
  ...
  #loop() {
    ...
    for (let i = 0; i < this.tweenList.length; i++) {
      ...
      if (i !== 0) {
        ...
        tween.timestamp =
          prevTween.timestamp + prevTween.duration * 1000 + tween.offset * 1000;
      }
      if (this.elapsedTime >= tween.timestamp) {
  ...
  fromTo(...args) {
    const duration = args[3]?.duration ?? 1;
    const offset = args[3]?.offset ?? 0;
